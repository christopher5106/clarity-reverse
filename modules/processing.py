...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    if p.scripts is not None:
        print("(process_images) before_process(p)")
        p.scripts.before_process(p)

    stored_opts = {k: opts.data[k] if k in opts.data else opts.get_default(k) for k in p.override_settings.keys() if k in opts.data}

    try:
        print("(process_images) try reload")
        ...
        print("(process_images) apply token merging ratio", p.get_token_merging_ratio())
        sd_models.apply_token_merging(p.sd_model, p.get_token_merging_ratio())

        print("(process_images) process_images_inner")
        res = process_images_inner(p)

    finally:
        print("(process_images) remove token merging")
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    if p.refiner_checkpoint not in (None, "", "None", "none"):
        print("(process_images_inner) get refiner")
    ...
    print("(process_images_inner) hijack apply circular + clear_comments")
    modules.sd_hijack.model_hijack.apply_circular(p.tiling)
    modules.sd_hijack.model_hijack.clear_comments()

    print("(process_images_inner) setup prompts")
    p.setup_prompts()
    ...
    if os.path.exists(cmd_opts.embeddings_dir) and not p.do_not_reload_embeddings:
        print("(process_images_inner) load textual inversion")
        model_hijack.embedding_db.load_textual_inversion_embeddings()

    if p.scripts is not None:
        print("(process_images_inner) process(p)")
        p.scripts.process(p)
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        with devices.autocast():
            print("(process_images_inner) p.init")
            ...
            print("(process_images_inner) apply unet")
            sd_unet.apply_unet()
        ...
        for n in range(p.n_iter):
            ...
            if p.scripts is not None:
                print("(process_images_inner) before_process_batch")
            ...
            if p.scripts is not None:
                print("(process_images_inner) process_batch")
            ...
            if n == 0:
                with open(os.path.join(paths.data_path, "params.txt"), "w", encoding="utf8") as file:
                    ...
                    #print(f"Parameters: {processed.infotext(p, 0)}")
            ...
            with devices.without_autocast() if devices.unet_needs_upcast else devices.autocast():
                print("(process_images_inner) p.sample")
            ...
            print("(process_images_inner) nextjob")
            state.nextjob()

            if p.scripts is not None:
                print("(process_images_inner) postprocess_batch")
                ...
                print("(process_images_inner) postprocess_batch_list")
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                print("(process_images_inner) batch", i)
                p.batch_index = i
                ...
                if p.restore_faces:
                    print("(process_images_inner) restore faces")
                ...
                if p.scripts is not None:
                    print("(process_images_inner) PostprocessImageArgs")
                    pp = scripts.PostprocessImageArgs(image)
                    p.scripts.postprocess_image(p, pp)
                    image = pp.image
                if p.color_corrections is not None and i < len(p.color_corrections):
                    if save_samples and opts.save_images_before_color_correction:
                        print("(process_images_inner) apply overlay")
                        image_without_cc = apply_overlay(image, p.paste_to, i, p.overlay_images)
                        images.save_image(image_without_cc, p.outpath_samples, "", p.seeds[i], p.prompts[i], opts.samples_format, info=infotext(i), p=p, suffix="-before-color-correction")
                    print("(process_images_inner) apply color correction")
                ...
                if hasattr(p, 'mask_for_overlay') and p.mask_for_overlay:
                    if opts.return_mask or opts.save_mask:
                        print("(process_images_inner) mask_for_overlay")
                    ...
                    if opts.return_mask_composite or opts.save_mask_composite:
                        print("(process_images_inner) image_mask_composite")
        ...
        if not infotexts:
            print("(process_images_inner) Processed(p, []).infotext")
        ...
        if (opts.return_grid or opts.grid_save) and not p.do_not_save_grid and not unwanted_grid_because_of_img_count:
            print("(process_images_inner) grid")
    ...
    print("(process_images_inner) res= Processed()")
    res = Processed(
        p,
    ...
    if p.scripts is not None:
        print("(process_images_inner) p.scripts.postprocess(p, res)")
